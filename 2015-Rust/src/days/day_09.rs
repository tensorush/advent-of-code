...
fn part_1(city_pairs: &str) -> u32 {
    ...
    let routes: Heap<Vec<&str>, &str> = Heap::new(&mut cities);
    ...
    let shortest_distance: u32 = route_distances.first().unwrap().clone();
    shortest_distance
}
...
fn part_2(city_pairs: &str) -> u32 {
    ...
    let mut route_distances: Vec<u32> = routes
        ...
        .collect();
    route_distances.sort();
    let longest_distance: u32 = route_distances.last().unwrap().clone();
    longest_distance
}
