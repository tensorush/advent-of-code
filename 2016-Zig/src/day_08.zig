...
fn Screen(comptime width: u8, comptime height: u8) type {
    return struct {
        ...
        fn parseAndExecuteInstructions(self: *Self, input: []const u8) std.fmt.ParseIntError!void {
            ...
            var size_split_idx: usize = undefined;
            var num_rotations: u8 = undefined;
            var rect_height: u8 = undefined;
            var rect_width: u8 = undefined;
            var x: u8 = undefined;
            var y: u8 = undefined;
            while (str_instruction_iter.next()) |instruction| {
                if (instruction[1] == 'e') {
                    size_split_idx = std.mem.indexOfScalar(u8, instruction, 'x').?;
                    rect_height = try std.fmt.parseUnsigned(u8, instruction[size_split_idx + 1 ..], 10);
                    rect_width = try std.fmt.parseUnsigned(u8, instruction[5..size_split_idx], 10);
                    self.rect(rect_width, rect_height);
                } else if (instruction[7] == 'r') {
                    y = try std.fmt.parseUnsigned(u8, instruction[13..14], 10);
                    num_rotations = try std.fmt.parseUnsigned(u8, instruction[18..], 10);
                    self.rotateRow(y, num_rotations);
                } else {
                    x = try std.fmt.parseUnsigned(u8, std.mem.trimRight(u8, instruction[16..18], " "), 10);
                    num_rotations = try std.fmt.parseUnsigned(u8, instruction[instruction.len - 1 ..], 10);
        ...
        fn rotateColumn(self: *Self, x: u8, num_rotations: u8) void {
            ...
            var y: u8 = undefined;
            while (rotation_count < num_rotations) : (rotation_count += 1) {
                last_pixel = &self.pixels[height - 1][x];
                y = 0;
                while (y < height) : (y += 1) {
                    std.mem.swap(bool, last_pixel, &self.pixels[y][x]);
                }
